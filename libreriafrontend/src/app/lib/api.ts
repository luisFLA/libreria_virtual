...
export const api = {
  ...
  obtenerFavoritosPorUsuario: (id_usuario: number) =>
        pedir<import("../Modelos/favorito").Favorito[]>(`/favoritos/usuario/${id_usuario}`),

  eliminarFavoritosPorUsuario: (id_favorito: number) =>
      pedir<{ message: string }>(`/favoritos/${id_favorito}`, { method: "DELETE" }),

  obtenerPedidosPorUsuario: (id_usuario: number) =>
        pedir<import("../Modelos/pedido").Pedido[]>(`/pedidos/usuario/${id_usuario}`),
